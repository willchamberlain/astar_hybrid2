...
%     plot3_rows(  reshape(z_eq_2_intercept, [121,3] )'+repmat([0;0;0],1,121)     , 'rx' )
   ...
   % NOW: cull these by (1) image formation + algorithm maximum resolution  (2) obstacles on floor plan  (3) non-free space
    ...
    grid_cells_best_payoffs_inverted = ...
        (ones(size(grid_cells_best_payoffs) ).*max(max(grid_cells_best_payoffs)) - grid_cells_best_payoffs).*2 + 0.1 ;
    
    
        figure_named('grid_cells_best_payoffs_inverted') ;  surf(grid_cells_best_payoffs_inverted) ; hold on; grid on; xlabel('x'); ylabel('y');  %   set(gca, 'ZScale','log')
    ...
    % todo: keeps punching through to the top-cost-but-shortest-path between the cells with payoffs : try spreading the payoff into the surrounding cells with some function.
    % NOW: check that a wall works 
    %     wall = [ 200:240 ; 200:240  ]'   
    %     grid_cells_best_payoffs_inverted(sub2ind(size(grid_cells_best_payoffs_inverted),wall(:,1), wall(:,2))) = 10000  ;
    %     grid_cells_best_payoffs_inverted(sub2ind(size(grid_cells_best_payoffs_inverted),wall(:,1), flip(wall(:,2),1))) = 10000  ;
    grid_cells_best_payoffs_inverted(220:230,220:230) = 10000  ;
    ...
    total_path_smoothed__ = ...
        ...
        path_planning__draw_path_yx( total_path_smoothed__ )    ;        
