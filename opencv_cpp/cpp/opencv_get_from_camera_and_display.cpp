/*
g++ opencv_get_from_camera.cpp -I/usr/include/opencv2 -L/usr/local/lib -lopencv_highgui -lopencv_core -lopencv_imgproc -lX11
...
void mouseCallback(int event, int x, int y, int flags, void* userdata) {
    ...
    if(event == CV_EVENT_LBUTTONDOWN) {
        ...
        user_data_ptr->sp->x = x;  user_data_ptr->sp->y = y;
...
int main(int argc, char const *argv[])
{
    ...
    cv::namedWindow("edges2",CV_WINDOW_NORMAL);
    ...
    cv::moveWindow("edges2", 10, 10);
    cv::resizeWindow("edges2", win_width, win_height);
    ...
    cv::setMouseCallback("edges2", mouseCallback, &user_data_);
