function pp = temp__pure_pursuit_vrep_waypoints( ...
    pioneer_z_ , ...
    pioneer2_ , ...
    way_points_ ,  vrep_ ,  pioneer_handle_  )

global stop_sim;
stop_sim=0;

pioneer_z  = pioneer_z_ ;
pioneer2 = pioneer2_ ;

load_pp = 0;
    if load_pp == 0
        pp.pioneer_z = pioneer_z ;
        ...
        pp.valmax = 10/100; % <== Max steering angle 
        pp.isStart = false;
        pp.sim_counter = 0;
...
%         end
        ...
        pause(0.01) ; 
        % fprintf('[%d] \n', loop.iter);
        if loop.iter >= loop.maxiter || stop_sim
            break;
        end
    end
end
...
pp.sim_d = val  ;
display(sprintf('setSteering(%4f) --> sim_d=%4f',val,pp.sim_d));
end

function pp = changeSteeringBy(pp, delta)
pp.sim_d = pp.sim_d + delta ;
display(sprintf('changeSteeringBy(pp, %4f) --> sim_d=%4f',delta,pp.sim_d));
end
...
nx = calcDrawX(pp_, pp_.steer_fx);
ny = calcDrawY(pp_, pp_.steer_fy);
d = calcDraw(pp_, pp_.FOLLOW_DIST); 
