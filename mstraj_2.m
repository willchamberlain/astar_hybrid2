...
function [TG, TGD, TGDD, taxis]  = mstraj_2(segments, qdmax, tsegment, q, dt, Tacc, varargin)
    ...
    for seg=1:ns
        ...
        [ qb , qbd , qbdd ] = jtraj(q, q_prev+tacc2*qd, 0:dt:taccx, qd_prev, qd);
        tg = [tg; qb(2:end,:)];
        tgd =  [tgd; qbd(2:end,:)];
        tgdd =  [tgdd; qbdd(2:end,:)];
        ...
        for t=tacc2+dt:dt:tseg-tacc2
            ...
            tgd =  [ tgd; (tg(end,:)-q)/dt  ];  % vel = posn diff/time step
            tg = [tg; q];
            tgdd =  [ tgdd; zeros(1,size(q,2)) ];   % linear: zero acceleration
            clock = clock + dt;
        end
    ...
    % add the final blend
    [ qb , qbd , qbdd ] = jtraj(q, q_next, 0:dt:tacc2, qd_prev, qdf);
    tg = [tg; qb(2:end,:)];
    tgd =  [tgd; qbd(2:end,:)];
    tgdd =  [tgdd; qbdd(2:end,:)];
