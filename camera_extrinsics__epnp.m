...
function [ ...
	    ...
	    extrinsic_estimate_as_world_to_local, ...
	    extrinsic_estimate_rotm_flu, ...
        solution_set, Alph] = ...
...
[R,T,Xc,best_solution, solution_set, Alph]=efficient_pnp_will( points_3d_in , points_2d_in , camera_K );             
            
% estimated camera-to-world frame --> estimated world-to-camera frame
extrinsic_estimate_as_world_to_local              =  vertcat(  horzcat(  R,   T  )   ,    [ 0 0 0 1]  ); 
extrinsic_estimate_as_local_to_world =  vertcat(  horzcat( R', (-(R')) * T )  ,   [ 0 0 0 1]  );            

est_rotm_rdf            = extrinsic_estimate_as_local_to_world(1:3,1:3)  ;
...
extrinsic_estimate_rotm_flu = est_rotm_flu_maybe;    % OUTPUT 
if draw_axes_
    draw_axes_direct( est_rotm_flu_maybe, extrinsic_estimate_as_local_to_world(1:3,4), '', 3, 'bob')
end
...
quat_flu_maybe_2222_wxyz  = rotm2quat( extrinsic_estimate_as_local_to_world(1:3,1:3)*rdf_in_flu );
 x=extrinsic_estimate_as_local_to_world(1,4);
 y=extrinsic_estimate_as_local_to_world(2,4);
 z=extrinsic_estimate_as_local_to_world(3,4);
...
struct_cam_pose_est(1,1).x=extrinsic_estimate_as_local_to_world(1,4);
struct_cam_pose_est(1,1).y=extrinsic_estimate_as_local_to_world(2,4);
struct_cam_pose_est(1,1).z=extrinsic_estimate_as_local_to_world(3,4);
