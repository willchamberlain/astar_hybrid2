...
%% --- Shut down VREP connection --- %%
vrep.simxStopSimulation(clientID,vrep.simx_opmode_oneshot);
pause(1);
vrep.simxFinish(clientID);
vrep.delete()
clear('vrep')
%% --- 2018_10_18 : set up callback(s) to move the cameras in VRep
    %  see  /mnt/nixbig/ownCloud/project_code/camera_extrinsics_data_analysis.m
    %  see  /mnt/nixbig/ownCloud/project_code/camera_extrinsics_data_analysis__keyboard_callback.m

fig_handle = figure_named('New UI to interact with VRep')            
            
set(fig_handle,'KeyPressFcn',@temp__vrep_drive_pioneer_2018_10_18__callback);
    % Later - move these into AppData / UserData / etc.
...
%%     the _dark arts_ of Matlab 'GUI' data/component interactions   -   ( THIS  WAS  OUT  OF  SEQUENCE )  %%

current_fig_handle = gcf  ;
figure__app_data_handles = guidata(current_fig_handle) ;      % GET   % handle to 'application data' of current callback object 
...
figure__app_data_handles.my_appdata_struct.vrep = vrep
figure__app_data_handles.my_appdata_struct.clientID = clientID 
figure__app_data_handles.my_appdata_struct.cam603_base_cuboid = cam603_base_cuboid



guidata( current_fig_handle , figure__app_data_handles );  % SAVE 

    
%% --------------------------------------------------------------------------------------------------------------------------
%  Start demonstration of getting and setting object poses   -  see  http://www.forum.coppeliarobotics.com/viewtopic.php?t=1445
    ...
    % PAN the camera - relative to the parent/cuboid frame
    [returnCode, orientation_cam603sensor_vs_base] = vrep.simxGetObjectOrientation(clientID, ...
        cam603_base_cuboid, ...
        cam603_base_cuboid, ...-1, ...cam603_base_cuboid, ...  cam603sensor, ...  -1, ...  %cam603_base_cuboid,...
        vrep.simx_opmode_oneshot_wait)
    eul2tr(orientation_cam603sensor_vs_base)    
    
     [returnCode]= vrep.simxSetObjectOrientation(clientID, ...
         cam603_base_cuboid, ...
         cam603_base_cuboid, ...-1, ...cam603_base_cuboid, ...  cam603sensor, ...  -1, ...  %cam603_base_cuboid,...
         tr2eul(  r2tr( rotz(degtorad(15)) )  *  eul2tr(orientation_cam603sensor_vs_base))  , vrep.simx_opmode_oneshot_wait)  ;
    
    
    
%  WEIRD! -  others work, this doesn't - time's well up; good try, but need to ship
    %--
    %  Get the handles for the  cam603sensor  : want to 
    %       apply x,y position changes to the  cam603_base_cuboid  relative to the world 
    %       apply orientation changes to the  cam603sensor  , relative to the  cam603_base_cuboid
    % TILT/PITCH the camera/sensor - uses LUF coordinate frame  - rotate around the X axis of the sensor   
    [retVal,cam603sensor] = vrep.simxGetObjectHandle(clientID,'cam603sensor',vrep.simx_opmode_oneshot_wait)  % get a handle to the object by name 

    [returnCode, orientation_cam603sensor] = vrep.simxGetObjectOrientation(clientID, ...
        cam603sensor, ...
        -1, ...cam603sensor, ... -1, ...  %cam603_base_cuboid,...
        vrep.simx_opmode_oneshot_wait)
    eul2tr(orientation_cam603sensor)    
    
     [returnCode]= vrep.simxSetObjectOrientation(clientID, ...
         cam603sensor, ...
         -1, ...cam603sensor, ...
         tr2eul(  r2tr( rotx(degtorad(-15)) )  *  eul2tr(orientation_cam603sensor))  , vrep.simx_opmode_oneshot_wait)  ;
      
    
            
    %--
    % Demonstrate moving  the base cube  object through a series of poses
    % 	[number returnCode]  (http://www.coppeliarobotics.com/helpFiles/en/remoteApiConstants.htm#functionErrorCodes)
    ...
    ii_ = 0
    for yaw = 90:-5:0
        ii_ = ii_ + 1;     rot_(:,:,ii_) = rotz(degtorad(yaw))  ;
    end  ;   size(rot_)  %  3  3  19
    eulerAngleZYZ = tr2eul(rot_)   ;  size(eulerAngleZYZ)   % 19  3
...
% Finished demonstration of getting and setting object poses
%%  ---------------------------------------------------------------------------------------------------------------------------
% open connection to VRep - see  http://www.forum.coppeliarobotics.com/viewtopic.php?t=3325
    vrep=remApi('remoteApi'); % using the prototype file (remoteApiProto.m)
    clientID=vrep.simxStart('127.0.0.1',19997,true,true,5000,5);  % get a connection = start a connection != start sim
...
idisp(map_image_2 )
%%  ---------------------------  finish loading the map from a map used for ROS
%%  ----------  start placing the cameras for one rendezvous run 
%- use bwdist to keep centre of FoV away from walls and small spaces
[D,IDX] = bwdist(1-map_image_2) ;
idisp(D)
map_image_good_spots = D<5*(1/resolution) & D>0.5*(1/resolution)   ;            %  for visualising map_image_good_spots(x,y)
map_image_good_spots_values = map_image_good_spots' ;                           %  for checking values map_image_good_spots_values(x,y)
idisp( map_image_good_spots )  ; hold on

%% -----------  check that the mapped values work / make sense in coordinate space 
idisp(map_image_good_spots)
for y = 1:20:size(D,1) ;     for x = 1:20:size(D,2) ; 
        hold on; plot(x,y,'bx')        
        if map_image_good_spots_values(x,y) >= 1  ;  plot(x,y,'gs')  ;  end
end ; end ;

%% -----------  pick a spot - start 
% draw - the map
    idisp(map_image_good_spots); hold on
% calculate - the first FoV centre
bad_points = []  ;
while true
    y = int16(randi(size(map_image_good_spots,1)))  ;
    x = int16(randi(size(map_image_good_spots,2)))  ;
    bad_points(:,end+1) = [x,y]  ;
...
FoV_centres = int16([])  ;
FoV_centres(1,:) = FoV_centre_1(1:2) ;
%%
...
while true
        ...
        x = x + FoV_centre_1(1);
        y = y+ FoV_centre_1(2);
        FoV_centre_maybe = [x y] ;
        if map_image_good_spots_values(x,y) < 1   
            hold on; plot(FoV_centre_maybe(1),FoV_centre_maybe(2),'bx')        
            display('Bad position: inside an obstacle: continuing')
            continue
            bad_one = false;
            bad_two = false;
            for ii_ = 1:size(FoV_centres,1) -1
                if bad_one 
                    display('bad_one == true : breaking out')
                    break
                end
                if norm_2(FoV_centres(ii_,:)-FoV_centre_maybe  , 2) < 5*(1/resolution)
                    hold on; plot(FoV_centre_2(1),FoV_centre_2(2),'bs')        
                    display('Bad position: too close to an existing point: breaking out of for loop')
                    bad_one = true;                        
                    break                                        
                end
            end
            if bad_one || bad_two
                display('Bad position: too close to existing FoV: continuing')
                continue
            end
        end
        class(FoV_centre_1)
        ...
        [x y]=bresenham(double(FoV_centres(1,1)), double(FoV_centres(1,2)), double(FoV_centre_2(1)), double(FoV_centre_2(2)))  ;
        ...
        if LoS_clear 
            ...
            FoV_centres(end+1,:) = FoV_centre_2(1:2) ;
            plot(x,y,'cs', 'LineWidth',2);
            break
        else
...
FoV_centres
% Plot the FoV centres found    
for ii_ = 2:size(FoV_centres,1)
    plot(  [FoV_centres(1,1), FoV_centres(ii_,1)] , [FoV_centres(1,2), FoV_centres(ii_,2)]  ) 
end
%% ------------------  Plot the FoV centres found    
for ii_ = 2:size(FoV_centres,1)
    plot(  [FoV_centres(1,1), FoV_centres(ii_,1)] , [FoV_centres(1,2), FoV_centres(ii_,2)]  ) 
...
radius=( (randi(5)/10)+1)*2*(1/resolution);
