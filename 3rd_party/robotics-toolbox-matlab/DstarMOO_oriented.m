%DstarMOO_oriented D*-MOO navigation class
%
...
%        goal = [50,30];
%        start=[20,10];
%        ds = DstarMOO_oriented(map);    % create navigation object
%        ds.plan(goal,1)        % create plan for specified goal
%        ds.path(start)         % animate path from this start location
% Example 2:
%       goal = [100;100];
%       start = [1;1];
%       ds = DstarMOO_oriented(0);       % create Navigation object with random occupancy grid
...
classdef DstarMOO_oriented < Navigation
    ...
    properties (SetAccess=private, GetAccess=private)
        ...
        normals_01      % add'l cost layer 01 (unused)
        normals_02      % add'l cost layer 02 (unused)
        normals_03      % add'l cost layer 03 (unused)
    ...
    methods
        ...
        % constructor
        function ds = DstarMOO_oriented(world, world_2, varargin)
            %DstarMOO_oriented.DstarMOO_oriented D*MOO constructor
            %
            % DS = DstarMOO_oriented(MAP, OPTIONS) is a D* navigation object, and MAP is an
        ...
        function reset(ds)
            %DstarMOO_oriented.reset Reset the planner
            %
        ...
        function s = char(ds)
            %DstarMOO_oriented.char Convert navigation object to string
            %
        ...
        function plot(ds, varargin)
            %DstarMOO_oriented.plot Visualize navigation environment
            %
        ...
        function plan(ds, goal, N, start_)
            %DstarMOO_oriented.plan Plan path to goal
            %
    ...
    num_cycles = 0;
        ...
        %DstarMOO_oriented.cost_get Get the specified cost layer
        ...
        %DstarMOO_oriented.distancemap_get Get the current distance map
        %
        ...
        %DstarMOO_oriented.costmap_get Get the current costmap
        %
        ...
        %DstarMOO_oriented.costmap_set Set the current costmap
        %
        ...
        %DstarMOO_oriented.costmap_modify Modify cost map
        %
        ...
        %DstarMOO_oriented.addCost Add an additional cost layer
        %
        ...
        function addNormal(ds, layer, values)
        %DstarMOO_oriented.addCost Add an additional cost layer  normals
        %
        % DS.addCost(layer,values) adds the matrix specified by values as a
        % cost layer.
        % Inputs
        ...
        %   values: normalized matrix the size of the environment (100x100)
            if size(values)~=size(ds.occgrid)
                display('addNormal: Error: layer size does not match the environment')
                return
            end
            if max(max(values))~=1 || min(min(values))~=0
                display('addNormal:Warning: layer values are not normalized [0:1]')
            end
            
            if layer==1
                ds.normals_01 = values;
            elseif layer==2
                ds.normals_02 = values;
            elseif layer==3
                ds.normals_03 = values;
            else
                display('addNormal: Layer index out of range')
            end
            % If more cost layers needed, add additional elseif statements
            % as above.
        end
            ...
            switch nargin
                ...
                case 3
                    %pt = [ds.cost_g(back_pointer) + 0.5*ds.traversal_cost(a,back_pointer);     % hard-coded explicit weighting: decrease the distance-to-goal cost: reduce the traversal cost
                          ...
                          [r,c] = ind2sub(size(ds.costmap), [ back_pointer ; a] )  ;  
                          vec = diff([r c]) ;
                    ...
                    pt = [ds.cost_g(back_pointer) + ds.traversal_cost(a,back_pointer);
                          ...
                          %abs( cos(    atan2(vec(2),vec(1)) - map_2(row_,col_)    ) )  ; 
%                           a
%                           back_pointer
%                           a-back_pointer   ;  [r,c] = ind2sub(size(ds.costmap), [ a ])  ; 
                          cell_cost_01;  %  ds.cost_01(a).*dir_cost_factor_1;  % *abs(cos(divergence))  -  divergence between normal_to_ray_from_camera and vector ( back_pointer to a ) : parallel to the normal is best (1), parallel to the ray is worst (2)
