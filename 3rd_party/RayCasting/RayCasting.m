...
%% Nested Functions
    ...
    function myMotionFcn
        if strcmp(myControl.type,'start') || (myControl.buttonDownFlag && ~isempty(myControl.type) && myControl.index > 0)
            ...
            % Create all rays
            x21 = zeros(allRays.num,3);
            y21 = zeros(allRays.num,3);
            x21(:,1) = sin(t)*allRays.radius*allRays.ab(1)+car.coord(1);
            y21(:,1) = cos(t)*allRays.radius*allRays.ab(2)+car.coord(2);
            x21(:,2) = sin(t)*car.Radius+car.coord(1);
            y21(:,2) = cos(t)*car.Radius+car.coord(2);
            x21(:,3) = NaN;
            y21(:,3) = NaN;
            x22 = reshape(x21',allRays.num*3,1);
            y22 = reshape(y21',allRays.num*3,1);
