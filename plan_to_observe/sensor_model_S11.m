...
size(floorplan)
figure; idisp(floorplan, 'ynormal')
...
world_x_extent_cells = [40 120]
world_y_extent_cells = [40 140]
...
%-----  First camera 
x_unit = [1;0]  ;
camera_1_angle_yaw = 90  ;
camera_position_1_world = [ 7.24 ; 3.89 ]  ;  % ;degtorad(camera_1_angle_yaw)]  ;
camera_position_1 = ROS_to_image_*camera_position_1_world.*cellls_per_metre  ;
camera_angle_rot2 = rotz(degtorad(camera_1_angle_yaw)) ;
camera_optical_axis_direction_unit_1 = camera_angle_rot2(1:2,1:2)*x_unit  ;
FoV_angle_1 = 62 ;
[distance_uncertainty_costs, check_dist_map, check_angle_map] ...
    = sensor_model_func(scale_up_, world_x_extent_cells, world_y_extent_cells, camera_optical_axis_direction_unit_1 , camera_position_1 , FoV_angle_1)  ;
%-----  Second camera 
camera_2_angle_yaw = 45  ;
camera_position_2_world = [ 7.23 ; 4.40 ]  ; %; degtorad(camera_2_angle_yaw)]  ;
camera_position_2 = ROS_to_image_*camera_position_2_world.*cellls_per_metre  ;
rot_z = rotz(degtorad(camera_2_angle_yaw));  rot_z = rot_z(1:2,1:2)  ;
camera_optical_axis_direction_unit_2 = rot_z*x_unit  ;   %camera_optical_axis_direction_unit_1  ;
FoV_angle_2 =62;
[distance_uncertainty_costs_2, check_dist_map_2, check_angle_map_2] ...
    = sensor_model_func(scale_up_, world_x_extent_cells, world_y_extent_cells, camera_optical_axis_direction_unit_2, camera_position_2 , FoV_angle_2)  ;
%-----  Third camera 
camera_3_angle_yaw = 270  ;
camera_position_3_world = [ 8.25 ; 3.85 ]  ;  % ; degtorad(camera_3_angle_yaw)]  ;
camera_position_3 = ROS_to_image_*camera_position_3_world.*cellls_per_metre  ;
rot_z = rotz(degtorad(camera_3_angle_yaw));  rot_z = rot_z(1:2,1:2)  ;
camera_optical_axis_direction_unit_3 = rot_z*x_unit  ;   % camera_optical_axis_direction_unit_1  ;
FoV_angle_3 =62;
[distance_uncertainty_costs_3, check_dist_map_3, check_angle_map_3] ...
    = sensor_model_func(scale_up_, world_x_extent_cells, world_y_extent_cells, camera_optical_axis_direction_unit_3, camera_position_3 , FoV_angle_3)  ;


%camera_1 = [ 10, 51 ]  ;    % expect extra contribution between camera_1 and (camera_2 | camera_3)
cameras(1,:)= round(camera_position_1);
...
cameras(2,:)=round(camera_position_2);
...
cameras(3,:)=round(camera_position_3);



hold on ;  plot2_rows([ camera_position_1 camera_position_2 camera_position_3 ].*100  , 'rx' )

        % visual check
        f_distance_uncertainty_costs=figure;  s=surf(distance_uncertainty_costs);  s.EdgeColor='None';   xlabel('x'); ylabel('y'); zlabel('uncertainty');  % axis equal; 
...
radtodeg(atan(camera_optical_axis_direction_unit_1(2)/camera_optical_axis_direction_unit_1(1)))
                ...
                x_unit = [1;0]  ;
                y_unit = [0;1]  ;
                x_unit_origin = [camera_position_1(1:2) x_unit+camera_position_1(1:2) ]  ;
                y_unit_origin = [camera_position_1(1:2) y_unit+camera_position_1(1:2) ]  ;
                x_unit_rotated = [camera_position_1(1:2) camera_angle_rot2(1:2,1:2)*x_unit+camera_position_1(1:2) ]  ;
                y_unit_rotated = [camera_position_1(1:2) camera_angle_rot2(1:2,1:2)*y_unit+camera_position_1(1:2) ]  ;

                camera_optical_axis_direction_unit_1 = camera_angle_rot2(1:2,1:2)*x_unit  ;
                camera_optical_axis_2units = [ camera_position_1(1:2) camera_position_1(1:2)+camera_optical_axis_direction_unit_1*2]  ;
                ...
                FoV_left_lim = [ camera_position_1(1:2) FoV_left_dir+camera_position_1(1:2) ]  ;
                FoV_right_lim = [ camera_position_1(1:2) FoV_right_dir+camera_position_1(1:2) ]  ;
                ...
                camera_position_h = e2h(camera_position_1(1:2))  ;
...
distance_uncertainty_costs = zeros(  world_x_extent_cells(2)*scale_up,world_y_extent_cells(2)*scale_up  )  ;
...
for xx_ = world_x_extent_cells(1):1:world_x_extent_cells(2)*scale_up
    for yy_ = world_y_extent_cells(1):1:world_y_extent_cells(2)*scale_up
       % angle of 2 relative to 1= atan2(v2.y,v2.x) - atan2(v1.y,v1.x)  
        diff_vec = [xx_ ;yy_] - camera_position_1(1:2).*scale_up  ;
       ...
       angle_diff = ...
        ...
        dist = norm_2(  [xx_ ;yy_] - camera_position_1(1:2).*scale_up , 1  )  ;
...
distance_uncertainty_costs_stacked_v1 = cat(3, 0.99*ones(size(distance_uncertainty_costs)), distance_uncertainty_costs, distance_uncertainty_costs_2)  ;
