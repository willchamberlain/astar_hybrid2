...
while norm_2(target_planned_path_end_posn-target_posn_start,1) > 0.0001
    ...
    if t > 1
        %  condensed calculations  : 
             dist__ = norm_2(target_posn_start-robot_posn,1)  ;
             dist_cell_to_target = dist__ ;     
             if strcmpi('repel',dist_cell_to_target_radius_type )
                 if dist_cell_to_target < dist_cell_to_target_avoid_radius_factor
                     dist_cell_to_target = -dist_cell_to_target;
                 end
             end
            cost__ =  0.75*(0.005275*exp(dist_cell_to_target*0.07015*(3/4) ))   ;  % parameterisation : exp(d*x) : x spreads the function
            cost__ =  cost__ * 1.1 ;
            %  costmap_obs_dist(xx_ , yy_) =  costmap_obs_dist(xx_ , yy_) * 1.1  ;
             if strcmpi('flat',dist_cell_to_target_radius_type )
                 if dist_cell_to_target < dist_cell_to_target_avoid_radius_factor
                     cost__ = 0 ;  %costmap_obs_dist(xx_ , yy_) =  0 ;
                 end
             end
            if cost__ > 1           %costmap_obs_dist(xx_ , yy_) > 1 
                cost__ = 1 ;        %costmap_obs_dist(xx_ , yy_) = 1  ;
            elseif cost__ < 0   %costmap_obs_dist(xx_ , yy_) < 0  
                cost__ = 0 ;        %costmap_obs_dist(xx_ , yy_) = 0  ;
            end
            
            costmap_obs_dist_scale_factor = 0.001;
            cost__ =  cost__ .* costmap_obs_dist_scale_factor;   % costmap_obs_dist = costmap_obs_dist .*  costmap_obs_dist_scale_factor;    
            cost__ =  cost__    ;  % costmap_total = costmap_obs_dist;
...
%     costmap_dist_to_path_follower(costmap_dist_to_path_follower>0.2)  ;
    ...
    costmap_total =  costmap_total + 1.0;  % do NOT know why I need this : need to add at least 1.0 to get A* to plan 
                            ...
                            %     hold on;  surf(costmap_dist_to_path_follower)  ;
                            %     hold on;  surf(costmap_total)  ;
